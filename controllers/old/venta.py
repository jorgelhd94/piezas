...
def seleccionar_pieza():
    ...
    crud = dict(header='', body=lambda row: DIV(A(SPAN("", _class="fa fa-dollar"),
                                                  _href=URL('realizar_venta', args=row.id),
...
def realizar_venta():
    ...
    # pieza=db(db.pieza.id==id).select()
    return locals()
...
def administrar():
    ...
    crud = dict(header='',
                body=lambda row: DIV(A(SPAN("", _class="icon magnifier icon-zoom-in glyphicon glyphicon-zoom-in"),
                                       _href=URL('detalles', args=[row.venta.id]),
                                       _class="btn btn-success btn-rounded"),
                                     A(SPAN("", _class="fa fa-pencil"),
                                       _href=URL('editar', args=[row.venta.id]), _class="btn btn-info btn-rounded"),
                                     A(SPAN("", _class="icon trash icon-trash glyphicon glyphicon-trash"),
                                       _href="#", _onclick="confirmDelete(" + str(row.venta.id) + ")",
...
def apiVenta():
    ...
    def POST(*args, **vars):
        ...
        if vars["operacion"] == 'info':
            pieza = db(db.pieza.id == vars["id"]).select()
        if vars["operacion"] == 'salida':
            ...
            if (auxFechaIn <= auxFechaOut):
                ...
                id_pieza_venta = db.pieza_venta.insert(codigo=vars["codigo"], nombre=vars["nombre"],
                                                       descripcion=vars["descripcion"], cantidad=vars["cantidad"],
                                                       unidad=vars["unidad"], precio_entrada=vars["precio_entrada"],
                                                       precio_salida=vars["precio_salida"],
                                                       fecha_entrada=vars["fecha_entrada"])
                ...
                proveedores = db(
                    (db.pieza_proveedor.id_pieza == vars["id"]) & (db.pieza_proveedor.id == db.proveedor.id)).select(
                    db.proveedor.ALL)

                for p in proveedores:
                    db.proveedor_venta.insert(
                        id_pieza_venta=id_pieza_venta, **p)

                db(db.pieza.id == vars["id"]).update(cantidad=total)
            else:
                errorFecha = "True"
        return locals()
